...
class HHCell: 
    ...
    def add_net_stim(self, interval, noise=0.2):
        netStim = h.NetStim()
        netStim.interval = interval
        netStim.noise = noise
        # netStim.start = 10
        # netStim.number = 10ee3
        nc = h.NetCon(netStim, self.synlist[-1]) # connect to the last synapse that exists for this cell
        nc.weight[0] = 0.01

        # to avoid objects deallocation:
        self.nslist.append(netStim)
...
# excitatory cells receive input from NetStims (with random rate)
for cell in popExc.cells:
    rate = np.abs(np.random.normal(10, 5)) # mean 10, std 5
    cell.add_net_stim(interval=1/rate * 1000) # Hz rate to millisec interval
